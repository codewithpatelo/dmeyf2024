...
FEintra_manual_creacionismoplus <- function( pinputexps )
{
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija


  param_local$meta$script <- "/src/wf-etapas/1301_FE_intrames_manual_creacionismoplus.r"
...
FErf_attributes_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$train$training <- c( 202102, 202103, 202104, 202105)
...
wf_competencia3_modelo4 <- function( pnombrewf )
{
  ...
  FEintra_manual_creacionismoplus()
