...
CN_canaritos_asesinos_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$train$training <- c( 202102, 202103)
...
HT_tuning_base <- function(pinputexps, bo_iteraciones, bypass = FALSE) {
  if (-1 == (param_local <- exp_init(pbypass = bypass))$resultado) return(0) # Línea fija bypass

  param_local$meta$script <- "/src/wf-etapas/2201_HT_xgboost_gan.r"

  # En caso de que se haga cross validation, se usa esta cantidad de folds
...
FM_final_models_xgboost <- function(pinputexps, ranks, qsemillas) {
  ...
  # Qué modelos quiero, según su posición en el ranking de la Bayesian Optimization, ordenados por métrica descendente
  param_local$modelos_rank <- ranks
  param_local$metrica_order <- -1 # ordeno por el campo métrica en forma DESCENDENTE

  # Qué modelos quiero, según su iteración bayesiana de la Bayesian Optimization, SIN ordenar
  param_local$modelos_iteracion <- c()

  # Clase y etiquetas positivas
  param_local$train$clase01_valor1 <- c("BAJA+2", "BAJA+1")
  param_local$train$positivos <- c("BAJA+2")

  # Generación de semillas con primos
