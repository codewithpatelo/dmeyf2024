...
# cargo el dataset
if(envg$PARAM$parquet) {
  envg$PARAM$dataset <- paste0( "./", envg$PARAM$input, "/dataset.parquet" )
} else {
...
if (envg$PARAM$parquet) {
  write_parquet(dataset, "dataset.parquet") 
} else {
  fwrite(dataset,
    file = "dataset.csv.gz",
    logical01 = TRUE,
...
if (envg$PARAM$parquet) {
  action_finalizar( archivos = c("dataset.parquet","dataset_metadata.yml")) 
} else {
