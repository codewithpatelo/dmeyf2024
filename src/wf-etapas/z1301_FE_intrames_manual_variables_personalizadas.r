...
calcular_media_movil <- function(variable, n = 3) {
  return(frollmean(variable, n = n, align = "right", fill = NA))
}

# Función para calcular la Transformada de Fourier
calcular_fourier <- function(variable) {
  fft_result <- fft(variable)
  return(Mod(fft_result))  # Retornar la magnitud
}
...
AgregarVariables_IntraMes <- function(dataset) {
  ...
  # Agregar media móvil a vm_mconsumospesos
if (atributos_presentes(c("vm_mconsumospesos"))) {
  dataset[, vm_mconsumospesos_media_movil := calcular_media_movil(vm_mconsumospesos, 3)]
}

# Agregar media móvil a vm_mconsumosdolares
if (atributos_presentes(c("vm_mconsumosdolares"))) {
  dataset[, vm_mconsumosdolares_media_movil := calcular_media_movil(vm_mconsumosdolares, 3)]
}

# Agregar media móvil a vm_msaldototal
if (atributos_presentes(c("vm_msaldototal"))) {
  dataset[, vm_msaldototal_media_movil := calcular_media_movil(vm_msaldototal, 3)]
}

# Agregar media móvil a vm_mpagado
if (atributos_presentes(c("vm_mpagado"))) {
  dataset[, vm_mpagado_media_movil := calcular_media_movil(vm_mpagado, 3)]
}
...
if (atributos_presentes(c("vm_msaldototal"))) {
  dataset[, vm_msaldototal_fourier := calcular_fourier(vm_msaldototal)]
}
